...
function AccountPanel(){
  ...
  const nextCopy=!status?.eligible?'Complete your first order to activate tier progression.':status?.next?`${Math.max(0,status.next.days-status.ageDays)} days until ${status.next.name}`:'Highest Bloodline tier reached.';
  return <div className="accountPanel">{profile.founder_number&&<div className="founderStrip">FIRST 100 — #{String(profile.founder_number).padStart(3,'0')}</div>}<div className="profileHead"><div><div className="eyebrow">L.C. MEMBER</div><h1>{profile.display_name||'Member'}</h1><p>{email}<br/>Member since {new Date(profile.created_at).toLocaleDateString()}</p></div><div className="statusStack"><div className="eyebrow">CURRENT STATUS</div><strong>{status?.current.name}</strong></div></div><div className="progress"><span style={{width:`${status?.progress??0}%`}}/></div><div className="progressMeta"><span>{status?.ageDays??0} days</span><span>{nextCopy}</span></div><div className="accountGrid"><div className="accountCard"><h2>Your access</h2><ul className="checkList"><li><span>Drop calendar</span><span>✓</span></li><li><span>Member previews</span><span>✓</span></li><li className={(status?.ageDays??0)<30||!status?.eligible?'locked':''}><span>Special colourways</span><span>30d</span></li><li className={(status?.ageDays??0)<90||!status?.eligible?'locked':''}><span>Accessories + prints</span><span>90d</span></li><li className={(status?.ageDays??0)<180||!status?.eligible?'locked':''}><span>Hidden products</span><span>180d</span></li><li className={(status?.ageDays??0)<365||!status?.eligible?'locked':''}><span>Archive editions</span><span>365d</span></li></ul></div><div className="accountCard"><h2>Collection history</h2><ul className="checkList"><li><span>Completed orders</span><span>{profile.order_count}</span></li><li><span>Lifetime spend</span><span>€{Number(profile.lifetime_spend||0).toFixed(2)}</span></li><li><span>Founder number</span><span>{profile.founder_number?`#${String(profile.founder_number).padStart(3,'0')}`:'—'}</span></li><li><span>First purchase</span><span>{profile.first_order_at?new Date(profile.first_order_at).toLocaleDateString():'—'}</span></li></ul></div></div><div className="buttonRow"><Link className="button red" href="/vault">Check Vault access</Link><button className="button secondary" onClick={signOut}>Sign out</button></div></div>;
}
...
function VaultPanel(){
  ...
  useEffect(()=>{(async()=>{try{const supabase=getSupabaseBrowserClient();const {data:{user}}=await supabase.auth.getUser();if(!user){setAccess({signedIn:false,unlocked:false,founder:false,days:0});return;}const {data}=await supabase.from('lc_profiles').select('created_at,first_order_at,founder_number').eq('id',user.id).single();if(!data){setAccess({signedIn:true,unlocked:false,founder:false,days:0});return;}const days=Math.max(0,Math.floor((Date.now()-new Date(data.created_at).getTime())/86400000));const founder=!!data.founder_number;setAccess({signedIn:true,unlocked:founder||(!!data.first_order_at&&days>=90),founder,days});}catch(e){setError(e instanceof Error?e.message:'Unable to connect.');}})()},[]);
  ...
  return <div className="vaultGate"><div className="lock unlocked">✓</div><div className="eyebrow">ACCESS GRANTED</div><h1>THE VAULT</h1><p>{access.founder?'First 100 status recognized.':'Bloodbound access recognized.'} Limited accessories, special prints and secret colourways will appear here when they release.</p><div className="vaultPreview"><div>ACCESSORIES</div><div>SPECIAL PRINTS</div><div>SECRET COLOURWAYS</div></div></div>;
}
