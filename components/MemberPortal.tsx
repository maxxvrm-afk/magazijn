...
function VaultPanel(){
  ...
  useEffect(()=>{(async()=>{try{const supabase=getSupabaseBrowserClient();const {data:{user}}=await supabase.auth.getUser();if(!user){setAccess({signedIn:false,unlocked:false,founder:false,days:0});return;}const {data}=await supabase.from('lc_profiles').select('created_at,first_order_at,founder_number').eq('id',user.id).single();const row=data as {created_at:string;first_order_at:string|null;founder_number:number|null}|null;if(!row){setAccess({signedIn:true,unlocked:false,founder:false,days:0});return;}const days=Math.max(0,Math.floor((Date.now()-new Date(row.created_at).getTime())/86400000));const founder=!!row.founder_number;setAccess({signedIn:true,unlocked:founder||(!!row.first_order_at&&days>=90),founder,days});}catch(e){setError(e instanceof Error?e.message:'Unable to connect.');}})()},[]);
